...
class TestFileStorage(unittest.TestCase):
    ...
    def test_store_first_object(self):
        ...
        self.assertTrue(
                os.path.exists(self.file_path), "file.json was not created")
        ...
        self.assertIn(
                key, data, "Saved object key not found in file.json"
                )

        # Verify the object's attributes
        saved_object = data[key]
        self.assertEqual(
                saved_object["id"], my_first_model.id, "ID does not match"
                )
        self.assertEqual(
                saved_object["name"], "First Model", "Name does not match"
                )
        self.assertEqual(
                saved_object["my_number"], 1, "my_number does not match"
                )
